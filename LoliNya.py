...
async def poke(ctx, user: discord.User = None):
    ...
    if not user:
        ...
        if user.mention == ctx.message.author.mention:
            ...
            pokeMsg = random.choice(["*Pokes {0}*. It's {1} fault, I-I'm innocent!".format(user.mention, ctx.message.author.mention),"Does this poke feels good {}, nya?".format(user.mention),"You have been poked from {}.".format(ctx.message.author.mention),"Sneak poke {0}! Will you poke me too {1}?".format(user.mention, ctx.message.author.mention)])
            await client.send_file(channel, io.BytesIO(response.raw.read()), filename="poke.gif", content=pokeMsg)
